...
def tally(target, guess):
    if len(target) != WORD_SIZE or len(guess) != WORD_SIZE:
...
def determine_status(response, turns):
    return Status.WON if response == [Match.EXACT] * WORD_SIZE else Status.LOST if turns == MAX_TRIES else Status.IN_PROGRESS


def create_message(turns, status, target):
    messages = ["Amazing", "Splendid", "Awesome", "Yay", "Yay", "Yay"]
    return messages[turns - 1] if status == Status.WON else "It was " +  target + ", better luck next time" if status == Status.LOST else ""
